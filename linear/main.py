...
def main():
    ...
    max_dtmc_size = 32
    # TODO: provare con sizes diverse, provare con HISTOGRAM_TOTAL_VAR e HISTOGRAM_TOTAL_JS?
    ...
    checkpoint_callback = ModelCheckpoint(dirpath=f"checkpoints/{name}", save_top_k=2, monitor="val_loss")
    trainer = pl.Trainer(max_epochs=500, accelerator="gpu", log_every_n_steps=1, logger=logger, callbacks=[checkpoint_callback])
